...
/** @construct
 * proof of work
 * block need to be added miner or computer in p2p network
 * need to sole math problem the one solve is the one has
 * the right problem to add block to chain
 *
 *
 **/
  ...
  isChainValid() {
    for (let i = 1; i < this.chain.length; i++) {
      ...
      if (currentBlock.previousHash !== previousBlock.calculateHash()) {
        console.error(
          "previous hash not right: " + JSON.stringify(currentBlock)
        );
        return false;
      }
    }
